...
% Regulation
%---------------------------------------------------------------------
REG_PTS_g6p         = reg(1);    % PTS   inhibition  by  g6p    (1:31) 
REG_PGI_6pg         = reg(2);    % PGI   inhibition  by  6pg    (2:32)
REG_PFK_pep         = reg(3);    % PFK   inhibition  by  pep    (3:33)
REG_PFK_adp         = reg(4);    % PFK   activation  by  adp    (3:34)
REG_PFK_amp         = reg(5);    % PFK   activation  by  amp    (3:35)
REG_PK_amp          = reg(6);    % PK    activation  by  amp    (10:36)
REG_PK_fdp          = reg(7);    % PK    activation  by  fdp    (10:37)
REG_PK_atp          = reg(8);    % PK    inhibition  by  atp    (10:38)
REG_G1PAT_fdp       = reg(9);    % G1PAT activation  by  fdp    (14:39)
REG_G6PDH_nadph     = reg(10);   % G6PDH inhibition  by  nadph  (21:40)
REG_PGDH_atp        = reg(11);   % PGDH  inhibition  by  atp    (22:41)
REG_PGDH_nadph      = reg(12);   % PGDH  inhibition  by  nadph  (22:42)
REG_PEPCxylase_fdp  = reg(13);   % PEPCxylase activatin by fdp  (12:43)
